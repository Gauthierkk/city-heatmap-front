...
data into this repo's `public/data/`, and commits + pushes so GitHub Pages
redeploys (decision #11). Valid `<city>` ids: `paris`, `nyc`, `austin`. To
refresh data manually, run the worker there (`python3 -m data fetch-stores --all
--out-dir ../city-heatmap-front/public/data`); see its README for the full
pipeline, guards, and runbook.
...
- Store data is pre-baked GeoJSON fetched at runtime from the app's own origin
  (decision #2). Files are nested per city: `public/data/<city>/food.geojson`
  (food — serves both Grocery and Specialty, split client-side by tag set) and
  `public/data/<city>/fitness.geojson` (fitness — lazy-loaded on first selection
  per city); `public/data/<city>/boundary.geojson` is the clip boundary. The
...
- The overlay is clipped to the city's administrative boundary
  (`public/data/<city>/boundary.geojson`, produced by the worker repo's
